...
| | 无 CDN | 有 CDN（本文） |
|--|-------|-------------|
| 静态资源位置 | 全在 Docker 容器内 | OSS + CDN 边缘节点 |
| Docker 镜像内容 | index.html + assets | 只有 index.html |
| 访问速度 | 取决于服务器带宽 | 全国 CDN 加速 |
| CI/CD 阶段数 | 2 个（build → deploy） | 3 个（build → upload-cdn → deploy） |
...
└───────────────────────────────────┘
  ↓
用户访问：
  index.html        ← 服务器 Nginx
  assets/js/css/img ← CDN（就近加速）
```
...
└───────────────────────────────────────────┘
      ↓
┌── deploy stage ───────────────────────────┐
│  rm -rf dist/assets/（已在 OSS）           │
│  docker build（精简镜像，只有 index.html）  │
